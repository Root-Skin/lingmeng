package com.lingmeng.crawler.utils;


import com.lingmeng.crawler.core.callback.DelayedCallBack;
import com.lingmeng.crawler.core.cookie.CrawlerCookie;
import com.lingmeng.crawler.core.cookie.CrawlerHtml;
import com.lingmeng.crawler.core.delayed.DelayedUtils;
import com.lingmeng.crawler.core.proxy.CrawlerProxy;
import com.lingmeng.crawler.factory.CrawlerProxyFactory;
import lombok.Data;
...
//    private static final ResourceBundle resourceBundle = ResourceBundle.getBundle("crawler");
    ...
    private SeleniumDriver initChromeDriver(Proxy proxy) {
        ...
        //创建service服务
        ChromeDriverService chromeDriverService = new ChromeDriverService.Builder().usingDriverExecutable(new File("D:/myproject/weblogic/chromedriver.exe")).usingAnyFreePort().build();
...
//        });
        handel(url, proxy, new ChromeCallback()  {
            @Override
            public void callBack(WebDriver webDriver) {
                webDriver.get(url);
                List<CrawlerCookie> crawlerCookieList = delayed(webDriver, cookieName);
                crawlerHtml.setCrawlerCookieList(crawlerCookieList);
                crawlerHtml.setHtml(webDriver.getPageSource());
            }
        });
    ...
    /**
     * 获取Cookie 的延时方法
     * 因为浏览器打开页面有可能js还没有执行完成，获取的数据是不准确定，
     * 通过有没有写入一些/
     * **特殊的cookie ( Set<Cookie> cookieSet = driver.manage().getCookies();)
     * 来判断页面是否已经加载完成
