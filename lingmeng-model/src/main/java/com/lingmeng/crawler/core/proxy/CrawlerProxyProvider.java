...
import com.lingmeng.crawler.core.callback.ProxyProviderCallBack;
import lombok.Data;
...
/**
 * 代理IP的提供者(这个类是为了随机获取一个可用的代理IP进行)
 */
@Data
@NoArgsConstructor
public class CrawlerProxyProvider {
    ...
    public CrawlerProxy getRandomProxy() {
        ...
        readLock.lock();
        try {
            if (isUsedProxyIp && !CollectionUtils.isEmpty(crawlerProxyList)) {
